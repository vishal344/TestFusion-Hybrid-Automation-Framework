...
public class RFQ_DetailsPage {
	...
	public void navigateToRFQPage() {
		...
		try {
			WebElement searchBox = wait.until(
					ExpectedConditions.visibilityOfElementLocated(By.id("search2")));
			searchBox.clear();
		...
		try {
			new WebDriverWait(driver, Duration.ofSeconds(10)).until(d -> d
					.findElements(By.xpath(
							"//table//tbody//tr[last()]//input"
							+ " | //table//tbody//tr[last()]//textarea"))
					.size() >= 1);
		} catch (TimeoutException ignored) {
	...
	public void uploadImageForRow(String imagePath, int dataLine) {
		...
		if (uploadLinks.isEmpty()) {
			System.out.println("  [Image] Upload link not found for data-line="
					+ dataLine + " — skipping");
			return;
		}
		...
		try {
			...
			String jsIntercept = "(function(){"
					...
					+ "if(this.type==='file'){"
					+ "var b64='" + base64Image + "',mime='" + mimeType
			...
			System.out.println("  [Image] Intercept failed: " + e.getMessage()
					+ " — trying Robot fallback");
			try {
				uploadLink.click();
				sleep(1500);
				...
				sleep(1500);
				dismissAlertIfPresent();
				System.out.println("  [Image] Robot fallback done for data-line=" + dataLine);
	...
	public void handleTargetPricePopup(String rawPrice, int dataLine) {
		...
		try {
			new WebDriverWait(driver, Duration.ofSeconds(15))
					.until(ExpectedConditions.elementToBeClickable(marginInputBy));
			System.out.println("  Modal fully loaded — inputs interactable");
		} catch (TimeoutException e) {
			System.out.println("  [WARN] Modal inputs slow — proceeding with 1s wait");
			sleep(1000);
		}

		// Re-fetch inputs fresh after waiting (DOM may have updated during animation)
		List<WebElement> modalInputs = driver.findElements(By.xpath(
				"//div[@id='customTargetPopupModal']//input[not(@type='hidden')]"));
		...
		for (WebElement btn : allBtns) {
			String cls = btn.getAttribute("class") != null
		...
		try { sleep(400); dismissToastrIfPresent(); } catch (Exception ignored) {}
		try {
			wait.until(d -> ((JavascriptExecutor) d)
					.executeScript("return document.readyState").equals("complete"));
		} catch (Exception ignored) {}
	...
	public void clickSendEmail() {
		...
		try {
			...
			System.out.println(
					"  [WARN] Version popup not detected — may have opened compose directly");
		}
	}
	...
	public void handleCreateVersionPopup() {
			...
			try {
				WebElement btn = new WebDriverWait(driver, Duration.ofSeconds(10))
						.until(ExpectedConditions.elementToBeClickable(
								By.xpath("//button[contains(text(),'Create Version')]")));
				((JavascriptExecutor) driver).executeScript("arguments[0].click();", btn);
		...
		if (!clicked) {
			try {
				WebElement btn = new WebDriverWait(driver, Duration.ofSeconds(10))
						.until(ExpectedConditions.elementToBeClickable(
								By.xpath("//button[contains(@class,'create-version')]")));
				((JavascriptExecutor) driver).executeScript("arguments[0].click();", btn);
		...
		if (!clicked) {
			System.out.println("  [WARN] All attempts failed — calling JS createVersion()");
			((JavascriptExecutor) driver).executeScript(
	...
	public void handleNotesPopup(String noteText) {
		...
		try {
			notesTextarea = longWait.until(
					ExpectedConditions.visibilityOfElementLocated(By.id("notesText")));
			System.out.println("  notesText is visible");
		} catch (TimeoutException e) {
			System.out.println(
					"  [WARN] notesText not visible after 60s — trying JS scroll fallback");
			try {
				...
				throw new RuntimeException(
						"Notes textarea not found after all attempts: " + e2.getMessage());
			}
		}
		...
		try {
			WebElement saveBtn = wait.until(
					ExpectedConditions.elementToBeClickable(By.id("saveNotes")));
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", saveBtn);
			System.out.println("  Notes Save clicked");
		} catch (Exception e) {
			System.out.println("  [WARN] saveNotes not found, trying JS: " + e.getMessage());
			((JavascriptExecutor) driver).executeScript(
	...
	public void selectVendors(String[] vendors) {
		...
		try {
			WebElement bccField = wait.until(
					ExpectedConditions.elementToBeClickable(By.id("bccField")));
			((JavascriptExecutor) driver).executeScript(
	...
	public void clickVendorSaveButton() {
		...
		try {
			...
			System.out.println("  [WARN] Button not found, calling JS: " + e.getMessage());
			((JavascriptExecutor) driver).executeScript(
	...
	public void clickSendEmailInCompose() {
		...
		try {
			WebElement sendBtn = wait.until(
					ExpectedConditions.elementToBeClickable(sendBtnBy));
			((JavascriptExecutor) driver).executeScript(
					"arguments[0].scrollIntoView({block:'center'});", sendBtn);
			sleep(200);
			...
			((JavascriptExecutor) driver).executeScript(
					"if(typeof sendMail==='function') sendMail();");
		}
		sleep(1500);
		dismissAlertIfPresent();
		waitForDom();
	...
	private void fillModalField(String labelText, String value) {
		...
		try {
			WebElement field = new WebDriverWait(driver, Duration.ofSeconds(15))
					.until(ExpectedConditions.elementToBeClickable(fieldBy));
			clearAndType(field, value);
			System.out.println("  " + labelText + " ← " + value);
		} catch (Exception e) {
			System.out.println("  [WARN] clickable wait failed for [" + labelText
					+ "], using JS fallback");
			try {
				((JavascriptExecutor) driver).executeScript(
						"var modal=document.getElementById('customTargetPopupModal');"
						+ "if(!modal) return;"
						...
						+ "    if(!input){"
						+ "      var fa=labels[i].getAttribute('for');"
						+ "      if(fa) input=document.getElementById(fa);"
						+ "    }"
						+ "    if(input){"
						+ "      input.value='" + value + "';"
						...
						+ "  }"
						+ "}");
				System.out.println("  " + labelText + " ← " + value + " (JS fallback)");
			} catch (Exception e2) {
				System.out.println("  WARNING: Could not fill [" + labelText
						+ "]: " + e2.getMessage());
			}
		}
	}
