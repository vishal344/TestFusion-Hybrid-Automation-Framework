...
public class RFQ_DetailsPage {
    ...
    /**
     * FIX: Dismiss any visible toastr/notification overlays before interacting.
     * The root cause of the ElementClickInterceptedException was a toastr <div>
     * blocking clicks on the textarea. This method:
     *  1. Waits up to 5s for the toastr to vanish on its own (common auto-dismiss).
     *  2. If still present, force-removes it via JavaScript so clicks go through.
     */
    private void dismissToastrIfPresent() {
        try {
            // Common toastr selectors — add more if your app uses different classes
            String toastrSelector =
                "div.toastr-message, div.toast, div.toast-message, "
                + "div.toastr, div#toast-container, div.jq-toast-wrap";

            // Wait up to 5s for natural auto-dismiss
            try {
                new WebDriverWait(driver, Duration.ofSeconds(5)).until(d -> {
                    List<WebElement> toasts = d.findElements(
                        By.cssSelector(toastrSelector));
     ...
     * Previously only alerts were dismissed, leaving toastr overlays able
     * to intercept clicks — causing ElementClickInterceptedException on td[17].
     */
    ...
    public void clickAddRow() {
        dismissAlertIfPresent();
        dismissToastrIfPresent();  // ← FIX: clear toastr before Add Row too
                ...
                for (XSSFShape shape : drawing.getShapes()) {
                    if (shape instanceof XSSFPicture) {
                        ...
                        String imgPath   = tempDir + File.separator
                            + "part_image_row" + excelRowIdx + "." + picData.suggestFileExtension();
